...
# ---------------------------------------------------------------------------
# Melhoria de resolução do timer no Windows
# ---------------------------------------------------------------------------

def _set_windows_timer_resolution() -> None:
    """
    Força o scheduler do Windows para resolução de 1 ms.

    Por padrão o Windows usa ~15 ms de resolução, o que torna tick_s < 0.015
    não confiável. Com timeBeginPeriod(1) o tick de 0.02 s passa a funcionar
    corretamente para τ ≥ 0.1 s.

    Sem efeito em Linux/macOS — a chamada é ignorada fora do Windows.
    """
    if sys.platform == "win32":
        try:
            ctypes.windll.winmm.timeBeginPeriod(1)
            log.info("Windows timer resolution: 1 ms (timeBeginPeriod ativado)")
        except Exception as exc:
            log.warning("Não foi possível ajustar timer Windows: %s", exc)
...
async def main_async(config_path: str) -> None:
    ...
    # Melhora resolução do timer antes de qualquer sleep
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Simulador de Usina Fotovoltaica — Version 3 (Windows)"
    )
    ap.add_argument(
        "--config",
