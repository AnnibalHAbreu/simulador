...
def encode_pf_u16(pf: float) -> int:
    """
    Codifica PF com sinal para U16:
    - PF positivo (0..1): valor = PF × 16384  (range 0..16384)
    - PF negativo (-1..0): valor = 65535 - |PF| × 16384  (range 49151..65535)
    """
...
class ModbusRtuPortServer:
    ...
    def _update_meter_registers(self):
        ...
        # offsets 0..13 conforme especificação
    ...
    async def run(self, tick_s: float):
        async def updater():
            # Atualiza registradores do medidor a cada ~100 ms
            # (independente do tick de simulação de 10 ms)
            update_interval = max(0.05, min(0.5, 0.1))
            while True:
