...
class StepProfile:
    """
    Perfil por degraus (piecewise constant).
    Lê CSV com colunas:
      - time_s (int)
      - value (float)  [ou mais colunas no caso de carga]
    """
...
class LoadProfile:
    ...
    def value(self, t_s: int, default_p_kw: float, default_q_kvar: float) -> Tuple[float, float]:
        if not self.points:
    ...
    def from_csv(path: str | Path) -> "LoadProfile":
        ...
        with open(path, "r", encoding="utf-8") as f:
            r = csv.DictReader(f)
            for row in r:
                pts.append((int(row["time_s"]), float(row["P_load_kW"]), float(row["Q_load_kVAr"])))
        pts.sort(key=lambda x: x[0])
